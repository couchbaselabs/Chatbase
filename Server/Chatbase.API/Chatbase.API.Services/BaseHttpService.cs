...
namespace Chatbase.API.Services
{
    public abstract class BaseHttpService
    {
        ...
        protected virtual async Task PutAsync<T>(string requestUri, T obj, CancellationToken cancellationToken = default(CancellationToken), Action<HttpRequestMessage> modifyRequest = null)
        {
            var jsonRequest = !obj.Equals(default(T)) ? JsonConvert.SerializeObject(obj) : null;
            var response = await SendAsync(HttpMethod.Put, requestUri, cancellationToken, modifyRequest, jsonRequest).ConfigureAwait(false);
